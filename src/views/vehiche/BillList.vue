    ...
    export default {
        ...
        methods: {
            ...
            index() {
                ...
                this.axios.get(`vehicle/bill/${this.id}`)
                    .then(response => {
                        this.ownerBillDetails = response.data.vehicles;
                        //    this.name=response.data.vehicleOwnersBills[0].name;
                        //    this.phone=response.data.vehicleOwnersBills[0].phone;
                        //    this.date=response.data.vehicleOwnersBills[0].created_at;
                        console.log(this.ownerBillDetails);
                    });
                // console.log(this.id)
            },
            voucherShow(item)
            {
                this.$router.push({name:'voucher.report',params:{id:item.vouchers_id}});
            }
        },
        mounted() {
            this.index();
